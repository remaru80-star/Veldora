...
ADMIN = 111111111,222222222        # Telegram user IDs (comma separated, no spaces)
DB_URL = mongodb+srv://user:pass@cluster0.mongodb.net/?retryWrites=true&w=majority
DB_NAME = my_bot_database
LOG_CHANNEL = -1001234567890       # Channel/group ID for start-up logs
START_PIC = https://i.imgur.com/example.jpg   # (optional) Image shown on /start
WEBHOOK = true                     # (optional) Set to "true" to use webhook mode
PORT = 8080                        # (optional) Required if WEBHOOK = true
Variable	Required	What it does
API_ID	✅	Telegram API ID – get it from my.telegram.org
API_HASH	✅	Telegram API hash
BOT_TOKEN	✅	Your bot token from @BotFather
ADMIN	✅	Comma‑separated Telegram user IDs that can use admin commands
DB_URL	✅	MongoDB connection string (local or Atlas) – stores user settings
DB_NAME	✅	Name of the MongoDB database to use
LOG_CHANNEL	✅	Telegram channel/group ID where the bot will send a startup message
START_PIC	❌	URL of a picture to show in the /start message (leave empty for text only)
WEBHOOK	❌	Set to true if you’re using a webhook (e.g. on a VPS with a public URL). Default = false (polling)
PORT	❌	Port for the webhook. Only needed if WEBHOOK = true.
